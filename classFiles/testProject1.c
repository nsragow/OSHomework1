...
void * producer(void *listenfdAddress){
	...
	for(hit=1; ;hit++) {
		logger(LOG,"producer","listenfd", listenfd);
		...
		logger(LOG,"producer","test cli_addr", length);
		logger(LOG,"producer","about to accept",0);
		if((socketfd = accept(listenfd, (struct sockaddr *)&cli_addr, &length)) < 0)
			logger(ERROR,"system call","accept",socketfd);		
		logger(LOG,"producer","socketfd: ",socketfd);
		...
		logger(LOG,"producer","got mutex lock\n",0);
			...
			*/
			...
			logger(LOG,"producer","requestBuffer.add", requestBuffer.add);
			requestBuffer.num++;
...
void * consumer(void * args){
	...
	while(1){
		pthread_mutex_lock(&m);
		logger(LOG,"consumer","got mutex",0);
		...
		logger(LOG,"consumer","requestBuffer.rem", requestBuffer.rem);
		currentRequest = requestBuffer.requests[requestBuffer.rem];
		requestBuffer.rem++;
		logger(LOG,"consumer","request removed", requestBuffer.rem);
		requestBuffer.num--;
		pthread_mutex_unlock(&m);
		pthread_cond_signal (&c_prod);
		logger(LOG,"consumer","unlocked mutex",0);
 /*
		switch (mode) {
   ...
   */
	...
	logger(LOG,"consumer","socketfd",currentRequest.socketfd);
...
int main(int argc, char **argv)
{
	...
	*/
	...
	logger(LOG,"main","listenfd", listenfd);
